...
from ..cutqc.wire_cutting_evaluation import run_subcircuit_instances
from ..cutqc.wire_cutting_post_processing import generate_summation_terms, build
from ..cutqc.wire_cutting_verification import generate_reconstructed_output


def cut_circuit_gates_and_wires(
    ...
    Args:
        ...
        double_wire_cost: Whether to double the cost of cutting a wire, i.e. using SWAPS and no classical communication
...
def find_gate_and_wire_cuts(
    ...
    Args:
        ...
        max_cuts: Max total number of cuts allowed
        num_subcircuits: List of number of subcircuits to try
        verbose: Whether to print information about the cut finding
        double_wire_cost: Whether to double count the cost of cutting a wire - true if using SWAPS and no classical comm
    ...
    my_n_vertices, my_edges, my_vertex_ids, my_id_vertices = my_result
    ...
    for num_subcircuit in num_subcircuits:
        if (
                num_subcircuit * max_subcircuit_width < num_qubits
                or num_subcircuit > num_qubits
                or max_cuts + 1 < num_subcircuit
        ):
            if verbose:
                print("%d subcircuits : IMPOSSIBLE" % (num_subcircuit))
            raise ValueError("The number of subcircuits is not possible given the input circuit.")

        kwargs = dict(
            n_vertices=my_n_vertices,
            edges=my_edges,
            vertex_ids=my_vertex_ids,
            id_vertices=my_id_vertices,
        ...
        if not feasible:
            ...
            # If using only SWAPs and no classical communication, use the weighted cost
            if double_wire_cost:
                cost = mip_model.weighted_cost
            else:
                cost = len(cuts)

            if cost < min_cost:
                min_cost = cost
                # best_mip_model = my_mip_model
                cut_solution = cuts
    return cut_solution
...
def _read_circuit_split_gate(
    circuit: QuantumCircuit,
) -> tuple[int, list[tuple[int, int]], dict[str, int], dict[int, str], Any]:
    """
    Read the input circuit to a graph based representation for the MIP model.
    ...
    my_vertex_ids = {}
    my_curr_node_id = 0
    my_qubit_gate_counter = {}
    for qubit in dag.qubits:
        my_qubit_gate_counter[qubit] = 0
    for vertex in dag.topological_op_nodes():
        if len(vertex.qargs) != 2:
            raise Exception("vertex does not have 2 qargs!")
        arg0, arg1 = vertex.qargs
        # Create two vertex names - one for each qubit in the gate
        vertex_name_1 = "%s[%d]%d" % (
            circuit.find_bit(arg0).registers[0][0].name,
            circuit.find_bit(arg0).index,
            my_qubit_gate_counter[arg0],
        )

        vertex_name_2 = "%s[%d]%d" % (
            circuit.find_bit(arg1).registers[0][0].name,
            circuit.find_bit(arg1).index,
            my_qubit_gate_counter[arg1],
        )

        my_qubit_gate_counter[arg0] += 1
        my_qubit_gate_counter[arg1] += 1
        # print(vertex.op.label,vertex_name,curr_node_id)
        for vertex_name in [vertex_name_1, vertex_name_2]:
            if vertex_name not in my_node_name_ids:  # and id(vertex) not in my_vertex_ids:
                my_node_name_ids[vertex_name] = my_curr_node_id
                my_id_node_names[my_curr_node_id] = vertex_name
                # my_vertex_ids[id(vertex)] = my_curr_node_id
                my_curr_node_id += 1

        # Add the edge between the two vertices. This edge is a result of two qubits being connected by a gate
        my_edges.append((my_node_name_ids[vertex_name_1], my_node_name_ids[vertex_name_2]))

    # Add the edges between vertices that are the same qubit but on sequential gates
    # Check how many gates are on each qubit using the qubit_gate_counter
    for qubit in my_qubit_gate_counter.keys():
        num_gates = my_qubit_gate_counter[qubit]
        if num_gates > 1:
            register = circuit.find_bit(qubit).registers[0][0].name
            qubit_index = circuit.find_bit(qubit).index
            for i in range(num_gates - 1):
                vertex_name_1 = "%s[%d]%d" % (register, qubit_index, i)
                vertex_name_2 = "%s[%d]%d" % (register, qubit_index, i + 1)
                my_edges.append((my_node_name_ids[vertex_name_1], my_node_name_ids[vertex_name_2]))

    # Do not need to add 1 because the node_id always increments post assignment
    my_n_vertices = my_curr_node_id

    my_result = (my_n_vertices, my_edges, my_node_name_ids, my_id_node_names)
    ...
    return n_vertices, edges, node_name_ids, id_node_names, my_result
